...
if (isset($_POST['time_in'])) {
  ...
  $student_id = $_POST['student_id']; // Replace with your method of obtaining the student ID
  $time_out = date("Y-m-d H:i:s"); // Get the current timestamp

  // Prepare and execute the SQL update statement

  $sql = "UPDATE students_tbl SET time_out = ? WHERE id = ?";
  // $sql1 = "UPDATE students_tbl SET time_in = ? WHERE id = ?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param("ss", $time_out, $student_id); // Assuming student_number is the correct column name
